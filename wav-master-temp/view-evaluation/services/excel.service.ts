...
export class ExcelService {
    ...
    exportSurveyToExcel(surveyform: EvaluationForm) {
        ...
        workbook.xlsx.writeBuffer().then((fdata) => {
            ...
            const filename = `${surveyform.agentname.replace(/ /g, '_').replace(/,/g, '_')}_${calldatetime}.xlsx`;
    ...
    addAgentInformation(worksheet: any, surveyform: EvaluationForm) {
        const topfooterRow = worksheet.addRow([`Evaluation report of ${surveyform.agentname} for callid ${surveyform.callid}`]);
