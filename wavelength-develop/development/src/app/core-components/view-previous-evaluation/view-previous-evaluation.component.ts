...
import { ViewEvaluationService } from '../../feature-modules/view-evaluation/services/view-evaluation.service';
import { ExcelService } from '../../feature-modules/view-evaluation/services/excel.service';
...
@Component({
  selector: 'app-view-previous-evaluation',
  templateUrl: './view-previous-evaluation.component.html',
  styleUrls: ['./view-previous-evaluation.component.css']
})
export class ViewPreviousEvaluationComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ViewPreviousEvaluationComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any
  ) { }

  ngOnInit() {
    try {
      this.getRouteParams(this.data);
  ...
  getSubmittedEvaluationForm() {
    try {
      this.isLoader = true;
      this.showEvaluationService.getPreviousEvaluationForm(this.evaluationId).takeWhile(() => this.alive)
        .pipe(
          finalize(() => {
            this.isLoader = false;
          }),
          ...
          data => {
            if (data && data['evaluationcallresults']) {
              ...
              //console.log(this.objEvaluationForm);
